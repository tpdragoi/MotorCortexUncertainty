...
% 1) R1 panel
subplot(2,2,kk)
imagesc( obj.time,               ... x axis
         1:nR1,                 ... y axis
         tnsorp_r1' );          ... data block
...
xlabel(['time from ' num2str(params.alignEvent)])
ylabel('Trial # (R1)')
title(sprintf('Date %s  Animal %s  Probe %d (R1)', ...
              obj.pth.dt, obj.pth.anm, kk))
xlim([-1 3])
set(gca,'FontSize',sz)

% overlay last‐lick lines on R1
hold on
for i = 1:nR1
    y = i;
    x = lastlickr1(i);
    line([x x],[y-0.5 y+0.5], ...
...
xlabel(['time from ' num2str(params.alignEvent)])
ylabel('Trial # (R4)')
title(sprintf('Date %s  Animal %s  Probe %d (R4)', ...
              obj.pth.dt, obj.pth.anm, kk))
xlim([-1 3])
set(gca,'FontSize',sz)

% overlay last‐lick and (optional) reward lines on R4
hold on
for i = 1:nR4
    y = i;
    x = lastlickr4(i);
    ...
    % if you want to show reward times uncomment:
    % r = rewardr4(i);
    % if r < 2
    %     line([r r],[y-0.5 y+0.5], ...
...
set(gcf, 'Position', [50 100 600 800]);  % [left bottom width height] in pixels
