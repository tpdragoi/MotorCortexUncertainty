...
date = '2025-02-04';
meta19 = loadTD2l_neur219(meta19,datapth,date);
date = '2025-02-05';
meta20 = loadTD2l_neur(meta20,datapth,date);
date = '2025-02-06';
meta21 = loadTD2l_neur(meta21,datapth,date);
date = '2025-02-07';
meta22 = loadTD2l_neur(meta22,datapth,date);
% TD8d R1

date = '2025-06-03';
meta23 = loadTD4l_neur(meta23,datapth,date);
date = '2025-06-04';
meta24 = loadTD4l_neur(meta24,datapth,date);
date = '2025-06-05';
meta25 = loadTD4l_neur(meta25,datapth,date);
date = '2025-06-06';
meta26 = loadTD4l_neur(meta26,datapth,date);
date = '2025-06-07';
meta27 = loadTD4l_neur(meta27,datapth,date);

date = '2025-08-21';
meta28 = loadTD5l_neur(meta28,datapth,date);
date = '2025-08-22';
meta29 = loadTD5l_neur(meta29,datapth,date);
date = '2025-08-23';
meta30 = loadTD5l_neur(meta30,datapth,date);
date = '2025-08-24';
meta31 = loadTD5l_neur(meta31,datapth,date);
date = '2025-08-25';
meta32 = loadTD5l_neur(meta32,datapth,date);
...
for c = conds2use
    ...
    elseif strcmp(obj.pth.dt,'2025-06-05') && strcmp(obj.pth.anm,'TDl4')
        condtrix(condtrix > 123) = [];     % elseif strcmp(obj.pth.dt,'2024-09-09') && strcmp(obj.pth.anm,'TD8d')
     %    condtrix(condtrix > 298) = [];
    end
...
% max_values = max(max(condpsth, [], 1), [], 2);
% condpsth = condpsth ./ max_values;
...
% Z-score the condpsth
% condpsth = (condpsth - mean_values) ./ std_values;
% Compute the mean across the second dimension after z-scoring
tnsorp1 = squeeze(mean(condpsth, 2));
...
% Kinematics1 = abs(Kinematics1);

% tnsorp1 = tensorprod(N1,Modes(:,1),3,1);
% tnsorp4 = tensorprod(N1,Modes(:,1),3,1);
...
% end_idx = 700;
% 
% % Extract activity in that window
% window_activity = tnsorp_r1(start_idx:end_idx, :);  % size: [61 × N]
% 
% % Compute total activity per neuron in that window
% activity_sum = sum(window_activity, 1);  % size: [1 × N]
% 
% % Sort neurons by activity (ascending or descending)
% [~, idx] = sort(activity_sum, 'descend');  % or 'ascend' if you want least active first
% 
% % Reorder tsnorp_1 accordingly
% tnsorp_r1 = tnsorp_r1(:, idx);
...
% Sort neurons by activity (ascending or descending)
[~, idx] = sort(activity_sum, 'ascend');  % or 'ascend' if you want least active first
...
for i = 1:size(tnsorp_r1,2)
    temp = P8(i);
    % [~, index_closest] = min(abs(obj.time - (last_nonzero_index(i) ) ));
    if ~last_nonzero_index(i) == 0
        ...
        if ~isnan(obj.bp.ev.lickL{i,1})
            ...
            licks = obj.bp.ev.lickL{i,1}; licks = licks(liks);
            if ~isnan(licks)
        rewardr1(i) = licks(1)- obj.bp.ev.goCue(temp);
            ...
            if ~isnan(obj.bp.ev.lickL{temp,1})
                ...
                licks = obj.bp.ev.lickL{temp,1}; licks = licks(liks);
                if ~isnan(licks)
                    rewardr1(i) = licks(1) - obj.bp.ev.goCue(temp);
                end
            end     
...
for i = 1:size(tnsorp_r4,2)
    ...
    if ~last_nonzero_index(i) == 0
        ...
        rewardr4(i) = obj.bp.ev.reward(temp)- obj.bp.ev.goCue(temp);
                ...
                if ~isnan(obj.bp.ev.lickL{i,1})
        rewardr41(i) = obj.bp.ev.lickL{i,1}(1)- obj.bp.ev.goCue(temp);
                end
        end
...
yy = 1.2;  % γ>1 packs more of the map toward the 0 end (blue)

% make a vector t that goes 0→1 but spends most of its time near 0
...
blackToWhite = [linspace(black(1), white(1), numColors)', ...
                linspace(black(2), white(2), numColors)', ...
                linspace(black(3), white(3), numColors)'];

numColors = 100;
black = [0, 0, 0];       % RGB for black
red   = [1, 0, 0];       % RGB for red

blackToRed = [linspace(black(1), red(1), numColors)', ...
              linspace(black(2), red(2), numColors)', ...
              linspace(black(3), red(3), numColors)'];
...
% 
% periodN = 'firstLick';
% imagesc(obj.time,[1:size(tnsorp_r1',1)], tnsorp_r1'); 
% % xline(0,'LineWidth', 1.5)
% colormap(cmp)
% caxis([rang]);
% colorbar; grid off; axis tight; 
% % axis square;
% box off;xlabel(['time from ', num2str(params.alignEvent)]);
% ylabel('Trial #'); 
% % title([kinfeat,' ','Projection (R1)'], 'Interpreter', 'none');
% % title('Mean Spike Rate R1');
% xlim([-0.3 3])
% title(['MSR R1 Date : ', num2str(obj.pth.dt  ),' Anm ',num2str(obj.pth.anm  ) ]);
% xline(0,'Color',[1 0 0 0.5],'LineWidth',2)
% 
% % Plot a vertical line at time=0 using xline
...
%     yval(i) = i;
%     xval(i) = lastlickr1(i);
%     line([xval(i), xval(i)], [yval(i)-0.5, yval(i)+0.5], 'LineWidth', 3, 'Color', [orange] );
% 
%         % rval(i) = rewardr1(i);
%         % 
%         % if rval(i) < 2
%         % 
%         % line([rval(i), rval(i)], [yval(i)-0.5, yval(i)+0.5], 'LineWidth', 3, 'Color', [pink]);
%         % end
% 
% end
% 
% xline(obj.time(firstLick(1)),'--','LineWidth', 2,'Color','b')
% xline(obj.time(firstLick(end)),'--','LineWidth', 2,'Color','b')

% rang = [-7 7];
sz = 20;

%%%%%%%%% Masking data after last lick time by setting to -1 %%%%%%%%%%%
masked_r1 = tnsorp_r1';  % Transpose so trials x time

for i = 1:size(masked_r1,1)
    if ~isnan(lastlickr1(i))  % Make sure the time is valid
        mask_idx = obj.time > lastlickr1(i);  % Find timepoints after last lick
        masked_r1(i, mask_idx) = -1;  % Set to -1 for black
    end
end

%%%%%%%%% Plotting %%%%%%%%%%%
rang = [-2 25];  % Extend lower limit to include -1
cmp = blueToWhite;  % Your original colormap (we’ll extend it)
% cmp = linspecer;
...
% Concatenate the data matrices vertically
...
% tnsorp_r1 = tnsorp_r1(1:1000,:);
% tnsorp_r4 = tnsorp_r4(1:1000,:);

% allData = [tnsorp_r1(:); tnsorp_r4(:)];
% p_all   = prctile( allData, [1 99] ); 
% 
% rang = [p_all(1) p_all(2)];  % Extend lower limit to include -1


preIdx      = obj.time < 0;
baselineMu  = mean( combined_data(:,preIdx) , 'all' );
dataZero    = combined_data - baselineMu;

imagesc(obj.time, 1:(nR1+nR4), dataZero);
colormap(cmp);

% now zero will sit at the bottom of your cmap, everything <0 = blueish
% and >0 will run up to your positive ceiling:
...
% Overlay vertical lines for last licks and rewards
pink = [1 0 0.68];
orange = [1 0.5 0.1];
...
for i = 1:nR1
    y = i;
    x = lastlickr1(i);
    line([x x], [y-0.5 y+0.5], 'Color', orange, 'LineWidth', 3);
end

% R4 lines (offset y-axis by nR1)
for i = 1:nR4
    y = nR1 + i;
    x = lastlickr4(i);
    line([x x], [y-0.5 y+0.5], 'Color', orange, 'LineWidth', 3);
    ...
    if task == 16
        r = rewardr4(i);
        r1 = rewardr41(i);
        % Optional pink reward lines:
        % if r < 2
        %     line([r r], [y-0.5 y+0.5], 'Color', pink, 'LineWidth', 3);
        % end
        % if r1 < 2
        %     line([r1 r1], [y-0.5 y+0.5], 'Color', pink, 'LineWidth', 2.5);
        % end
    else
        r = rewardr4(i);
        % if r < 2
...
set(gcf, 'Position', [50 100 800 200]);  % [left bottom width height] in pixels
