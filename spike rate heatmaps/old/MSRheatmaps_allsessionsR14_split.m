...
% baseline_mean = mean(tnsorp_r1(1:300, :), 1);  % 1 x 136 vector
% 
% % Subtract the baseline mean from each trial's time series
% tnsorp_r1 = tnsorp_r1 - baseline_mean;
% 
% baseline_mean4 = mean(tnsorp_r4(1:300, :), 1);  % 1 x 136 vector
% 
% % Subtract the baseline mean from each trial's time series
% tnsorp_r4 = tnsorp_r4 - baseline_mean4;
...
% rang = [min(tnsorp_r4,[],'all') max(tnsorp_r4,[],'all')];  % Extend lower limit to include -1
...
set(gcf, 'Position', [50 100 900 500]);  % [left bottom width height] in pixels
