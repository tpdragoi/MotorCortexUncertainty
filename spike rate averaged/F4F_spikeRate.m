%% F4F_spikeRate.m
%  Session-normalized mean spike rate across units, VTA Reward Task.
...
cfg.minTrials = 1;   % was 5 -- forScience kept every session with any trial
...
cfg.normLastContact = 6;   % the trim ends at this contact
cfg.normMode        = 'trace';   % was 'window'

% TRIAL RULE. A trial is used only if it has at least cfg.boutMinContacts
% port contacts (lickL) in (goCue, goCue + cfg.boutWin_s]. Pre-cue licks do not
% count. Applied on top of the condition set (condition 1 = every trial in VTA).
cfg.boutMinContacts = 4;
cfg.boutWin_s       = 1.5;   % 'window' (above) | 'trace' (whole trace) | 'none' (Hz)
...
cfg.traceNorm        = 'trace';   % was 'window': min-max over the whole trace, as forScience
cfg.traceNorm_tongue = true;   % rescale the tongue trace to [0,1] for display

%% DISPLAY SMOOTHING
% As main_spikeRate_manySessions_VTA_forScience.m: after averaging across
% sessions, the mean rate is smoothed with movmean(., 13), the 95% CI half-width
% with movmean(., 13), and the mean tongue with movmean(., 25). Samples of 5 ms;
% rescaled to keep the same duration if params.dt changes.
cfg.dispSmooth       = true;
cfg.dispSmoothRate   = 13;
cfg.dispSmoothTongue = 25;
cfg.dispSmoothCI     = true;
...
spec.groupLabels  = {'M1'};
spec.groupColours = [0.10 0.60 0.25];   % VTA green
spec.trialCaps           = { 'TDv1','2025-02-15', 211 };
spec.singleProbeSessions = { 'TDv1','2025-02-15' };
% CONDITION 1, not 8. This cohort has no 'hit==1 & rewardedLick==1' trials --
% condition 8 selects nothing here, which is why it must match the VTA decoding
% script's spec.poolCondIdx = 1 rather than R1's 8.
spec.condSets = struct('name', {'all'}, 'condIdx', {1}, ...
                       'colour', {[0.10 0.60 0.25]});   % VTA green
spec.showEpochFigure = false;
spec.compareGroups   = [];
%% MAIN LOOP (shared)
...
fprintf('  [forSci] trials: >= %d port contacts within %.1f s after the go cue\n', cfg.boutMinContacts, cfg.boutWin_s);
...
% contacts in (goCue, goCue + boutWin_s] per trial -> the trial rule
nEarly = zeros(maxUsable,1);
for tr = 1:maxUsable
    lk = S.lickL{tr};
    if isempty(lk), continue; end
    nEarly(tr) = sum(lk(:) > S.goCue(tr) & lk(:) <= S.goCue(tr) + cfg.boutWin_s);
end
boutOK = hasContacts & nEarly >= cfg.boutMinContacts;
...
for cs = 1:nCS
    ...
    nCond  = numel(trials(hasContacts(trials)));
    trials = trials(boutOK(trials));
    logf('  [trials] sess %2d probe %d: %d of %d trials have >= %d contacts in %.1f s after the go cue\n', ...
        sessionIdx, probeIdx, numel(trials), nCond, cfg.boutMinContacts, cfg.boutWin_s);
...
% itself a trial average.
    ...
    needWin = any(strcmpi({cfg.normMode, cfg.traceNorm}, 'window'));
    if needWin && (~isfinite(gcMed) || ~isfinite(cMed(cfg.normLastContact)))
...
for g = 1:nG
    ...
    for cs = 1:nCS
        ...
        yyaxis(ax,'left')
        [m, ci] = meanCI(R);
        [wR, wT] = dispSmoothWidths(median(diff(tCommon)), cfg);
        m = movmean(m, wR);
        if cfg.dispSmooth && cfg.dispSmoothCI, ci = movmean(ci, wR); end   % CI smoothed too
        ylL = padLim([m - ci; m + ci]);   % y limits as forScience
        gd = isfinite(m) & isfinite(ci);
        if any(gd)
            fill(ax, [tCommon(gd); flipud(tCommon(gd))], [m(gd)+ci(gd); flipud(m(gd)-ci(gd))], ...
                col, 'FaceAlpha',0.3, 'EdgeColor','none', 'HandleVisibility','off');   % alpha 0.3
        end
        h(end+1) = plot(ax, tCommon, m, '-', 'Color', col, 'LineWidth', 3.5);   %#ok<SAGROW> lw 3.5
        ylim(ax, ylL);
        lbl{end+1} = sprintf('%s rate (n=%d)', spec.condSets(cs).name, size(R,2));   %#ok<SAGROW>
        fprintf('  [forSci] %s %s: %d sessions -> rate window %d, tongue window %d samples\n', ...
            spec.groupLabels{g}, spec.condSets(cs).name, size(R,2), wR, wT);
        nUsed = max(nUsed, size(R,2));

        yyaxis(ax,'right')
        T = stackCols(res(rows), 'tongue');
        if cfg.traceNorm_tongue, T = normaliseCols(T, 'minmax'); end
% tongue traces are greyscale, never the condition colour: the
% first trial type is black and any further ones step towards grey.
        if nCS > 1
            tcol = repmat(0.55*(cs-1)/(nCS-1), 1, 3);
        else
            tcol = [0 0 0];
        end
        mt = movmean(mean(T,2,'omitnan'), wT);
        h(end+1) = plot(ax, tCommon, mt, '-', ...   % solid, lw 3.5
            'Color', tcol, 'LineWidth', 3.5);   %#ok<SAGROW>
        ylim(ax, padLim(mt));
...
function [wRate, wTongue] = dispSmoothWidths(dt, cfg)
% Fixed moving-average widths of main_spikeRate_manySessions_VTA_forScience.m
% (winSR1 = 13, winMov1 = 25 samples of 5 ms), rescaled to params.dt.
    if ~cfg.dispSmooth || ~isfinite(dt) || dt <= 0
        wRate = 1;  wTongue = 1;  return
    end
    s = cfg.dispSmoothRefDt / dt;
    wRate   = max(1, round(cfg.dispSmoothRate   * s));
    wTongue = max(1, round(cfg.dispSmoothTongue * s));
end

function yl = padLim(v)
% [min max] of v padded by 5% of the range, as in forScience.
    v = v(isfinite(v));
