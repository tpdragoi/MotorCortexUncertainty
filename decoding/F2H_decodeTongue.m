...
cfg.fitWindowMode     = 'contact1';   % EARLY model = Fig. 2H (was 'lick2ToBoutEnd'). Options: 'contact1' | 'bout' | 'lick2ToBoutEnd'
...
cfg.ridgeImpl = 'eig';   % 'eig' | 'matlab' -- the SAME estimator; see
% section 1c in the header. 'matlab' calls
% squares problem separately for every lambda:
% 50 grid points x 6 paths per fit = 300 QR
% factorisations of an (n+p) x p matrix, per
% probe. 'eig' does one eigendecomposition per
% path and the whole grid as one matrix
% product. Set 'matlab' to reproduce, 'eig' to
% run; cfg.ridgeCheckEquivalence proves they
% agree.
% THE REPORTED WEIGHTS COME OUT OF MATLAB'S ridge, LITERALLY.
% The lambda path and the cross-validation folds are internal machinery: they
% choose lambda and they draw the diagnostic curves, and no weight from them is
% ever reported. So run those with 'eig' (fast), then REFIT ONCE at the chosen
% lambda with ridge() itself and use those coefficients for the prediction, the
% decoding index and everything downstream.
% Cost: ONE augmented least-squares solve per probe fit instead of 300. The
% Methods sentence "weights were estimated by ridge regression (using the MATLAB
% function 'ridge')" is then a plain description of what the script did.
% Every fit prints how far the refit moved the coefficients. Expect ~1e-13.
cfg.ridgeFinalMatlab = false;   % no ridge() call at all; the closed form
% IS the reported fit (verified identical)

% ---- THE VERIFICATION ----------------------------------------------------
% On the FIRST probe fit, run the whole comparison on this session's own
% design matrix and print a verdict: coefficients, fitted values, and the
% cross-validated lambda, closed form vs ridge(). Subsampled so the slow
% route is affordable -- about a minute at the defaults below.
...
%% A/B: BOTH RIDGE ROUTES, ON EVERY PROBE FIT
% Every probe fit is done TWICE, on the same design matrix, the same rows and
% the same lambda grid:
%   ROUTE M   the MATLAB function ridge(y, X, lambdas, 0)
%   ROUTE C   the same estimator written out inline at the fit site below --
%             no local function, no helper, so the thing being compared is
%             visible in one place next to the call it is being compared with
% Route M is AUTHORITATIVE: the b0/beta that go on to the decoding index and
% into res() come from ridge(). Route C is the challenger and is carried all
% the way through to its own decoding index, using ITS OWN cross-validated
% coefficient vectors agree at a lambda one of them did not choose.
% WHAT IT COSTS, plainly. ridge() does one augmented least-squares solve PER
% LAMBDA, so a 50-point grid is 50 QR factorisations of an (n+p) x p matrix per
% path; with cfg.abCompareCV that is SIX ridge() paths per probe fit (the full
% training path plus five cross-validation folds). An A/B run therefore takes
% about as long as the all-'matlab' script did, plus the closed form on top.
% says whether the cost has to be paid again.
cfg.abRidge      = false;   % A/B DONE -- verified identical on 8 fits.
% true re-runs both routes and re-prints the
% per-session comparison.

cfg.abCompareCV  = true;   % also run the five-fold lambda selection BOTH ways.
% This is the expensive half -- five extra ridge()
% paths per fit -- and it is the ONLY part that can
% show a genuinely DIFFERENT LAMBDA rather than
% round-off in the coefficients. false = compare the
% lambda paths only and let route M's lambda stand

cfg.abReportFile = fullfile(tempdir, 'ridgeAB_tongue_r14_clean.mat');

% THE YARDSTICK for the absolute differences printed below. The target is tongue
% length normalised to 0-1 (cfg.normPctLow/High), so an absolute difference is
% already a fraction of full protrusion and needs no conversion. This is only
% what it gets COMPARED AGAINST in the printout -- roughly what one camera pixel
% scales a diagnostic message and nothing numerical.
cfg.abPixelFrac = 0.0075;   % ~1 camera pixel as a fraction of full protrusion

if cfg.abRidge
% Both of these are superseded by the A/B and would only duplicate work:
% the equivalence check ran on the first session, the final refit existed to
% get ridge() weights out of an 'eig' run. Here ridge() IS route M, on every
% fit, and its weights are the ones reported.
...
% decoding-index curve.
spec.poolCondIdx = [8 9];
spec.splitRule   = 'typeStratified';
spec.trainFrac   = 0.60;   % Methods: ~60% for the Delayed Reward task
...
spec.tsColours  = [0.00 0.00 0.55 ;   % R1  dark blue
                   0.35 0.70 0.95];   % R4  light blue
spec.grpColours = [];
spec.predColour = [0.00 0.00 0.55];   % prediction on the diagnostics
...
spec.figRef       = 'Fig. 2H';   % the manuscript panel this file produces
...
spec.summaryFile  = fullfile(tempdir, 'decodeSummary_tongue_r14_clean.mat');
...
% old repeat loop: for kkkkk = 1:1  -> now RUN.nRepeats at the top
...
% predictors and tens of thousands of rows that is gigabytes, and the original
% built it THREE TIMES on every fit (equivalence check, fit, cross-validation).
...
% coefficients on the scale of the ORIGINAL predictors, and returns a
% (p+1) x nLam matrix with the INTERCEPT IN ROW 1. With ridge's default
% cannot predict from raw X -- and nothing errors, so the mistake is silent and
% the numbers are simply wrong by whatever the predictor scales happen to be.
% Internally this builds [Z; sqrt(lambda)*I], an (n+p) x p matrix, and calls
% mldivide on it SEPARATELY FOR EVERY LAMBDA. That is where the time goes.
...
% ---- ROUTE C: the same estimator, written out, no function call ---------
% Centre and scale to unit standard deviation, because ridge() always penalizes
% the STANDARDIZED coefficients and always leaves the intercept unpenalized; a
% route that did anything else would not be comparable, it would be a different
% estimator. Then form the Gram matrix ONCE, eigendecompose it ONCE, and read
% the entire lambda path off that one decomposition:
% with Z the centred-and-scaled design, and (d, V) the eigenvalues and
% eigenvectors of Z'Z. Vc ./ (d + lamGrid) is p x nLam by implicit expansion, so
% the whole grid is a SINGLE matrix product -- one BLAS-3 call in place of nLam
% separate solves. Algebraically identical to the augmented least-squares
% problem route M solves; the order of the arithmetic is the only difference,
% and it is the entire source of the discrepancy printed below.
% ridge replaces the standard deviation by 1 whenever abs(std) < sqrt(eps)
% (~1.5e-8), not only when it is exactly zero. A unit that fired in only one or
% two window samples can easily land between those two thresholds, and then a
% route that tests `== 0` divides that column by 1e-10 while ridge() divides it
% by 1 -- an enormous coefficient difference that has nothing to do with the
% arithmetic being compared. Matching the threshold keeps this A/B a test of
% the ORDER OF OPERATIONS, which is the only thing that should differ.
% (NOTE: ridgePath's own 'eig' branch had the `== 0` form; it is corrected to
% match lower down, so the cross-validation folds are comparable too.)
...
% edf comes free here: the eigenvalues of the standardized Gram matrix ARE the
% squared singular values of the standardized design, so the svd that ridgePath
% would otherwise run -- at roughly the cost of the whole fit -- is not needed.
R.edf  = 1 + sum(dC(:) ./ (dC(:) + lamGrid), 1);
Rc.edf = R.edf;
clear VC VcC

% ---- WHAT THE TWO PATHS DISAGREE ABOUT ----------------------------------
% Coefficients AND fitted values, because they can come apart. A predictor with
% no variance inside the fitting windows is the case: ridge() substitutes a
% column of ones and penalizes it, route C leaves it at zero, and the
% coefficients then differ visibly while the predictions do not move at all,
% because such a column contributes nothing either way. The fitted values are
% what every reported number is built from, so they are the comparison that
% decides anything.
% (:) rather than max(...,[],'all') so this still runs on R2016b, which is the
% floor the rest of the file already sets.
...
% UNDER THE A/B this rule is run TWICE over the SAME fold assignment
% (cfg.cvSeed fixes it), once with the folds fitted by ridge() and once
% with them fitted by the closed form. Only the fitting route inside the
% folds differs. Note what is NOT being compared: ridge() does not
% choose lambda, it takes lambda as an input argument, so there is no
% MATLAB built-in for the selection rule to be "the same as". The rule
% is the one in the Methods, it is selectLambdaCV5, and it is the same
% code in both arms.
...
% the curve the diagnostic figure draws under panel 1
    ...
    case 'gcv'
        [kSel, gcv] = selectLambdaGCV(ytr, Ptr(okTr,:), R.edf, cfg);
        kM = kSel;  kC = kSel;  sseM = gcv;  sseC = gcv;
        tCvM = NaN;  tCvC = NaN;
...
% Route C's own end-to-end answer: ITS lambda, ITS weights. If kC and kSel are
% the same grid point this is a round-off comparison; if they are not, it is the
% comparison that matters, and the decoding-index difference below is the real
% cost of switching routes rather than a number of digits.
if cfg.abRidge
    b0C = Rc.b0(kC);  betaC = Rc.beta(:,kC);
end

% ---- REFIT AT THE CHOSEN LAMBDA WITH MATLAB'S ridge ----------------------
% One lambda, one solve. These are the weights that produce every reported
% number from here on.
...
% Everything below is printed on the scale it will be read on. The target is
% tongue length normalised to 0-1, so an absolute difference IS a fraction of
% full protrusion and needs no conversion. The decoding index is the number that
% ends up on the figure, quoted to three decimals.
if cfg.abRidge
% THE PREDICTION DIFFERENCE, SAMPLE BY SAMPLE, over every test trial and
% every time point in the analysis window. A max on its own is the right
% conservative bound but it is one sample out of millions and says nothing
% about the typical size, so the spread is reported with it: if the max is
% the whole trace.
...
% THE DECODING INDEX, PER CONTACT. This is the number that reaches the
% single worst case.
...
% argmin can only move if the round-off between the two held-out error
% curves exceeds the gap between neighbouring grid points on that curve.
    if cfg.abCompareCV
...
% One row per probe fit, then the totals. This is the table that answers the
% and if not, by how much and on what scale.
...
% Run this once and the next optimisation aims itself. 'load' is zero on a warm
% cache; 'design' is laggedDesign building the n x (units*taps) matrix; 'fit' is
% the ridge path; 'lambda' is the five folds; 'figures' is everything drawn.
    tTotal = toc(tWall);
    fprintf('\n%s\n', repmat('=',1,60));
...
% Inline-able two-way pick, so the fprintf lines above stay one line each.
...
% so the cross-validation folds pass wantEdf = false and skip it.
    ...
    switch impl
        ...
        case 'eig'
% Algebraically identical, from ONE eigendecomposition of the Gram
% matrix rather than one augmented least-squares solve per lambda.
% MATLAB's ridge forms [Z; sqrt(k)*I] and calls mldivide separately
% dozens of QR factorisations; this is one eig and nL cheap solves.
            mx = mean(X,1);  my = mean(y);
            Xc = X - mx;     yc = y - my;
            if standardizeCols
% Same threshold MATLAB's ridge uses (see the A/B fit block):
% this branch are comparable with the folds fitted by ridge().
                sx = std(X,0,1);
...
% rather than recomputing them with an svd that costs as much as
% the entire fit.
            dGram  = d;
    ...
    if wantEdf
        if ~isempty(dGram)
% 'eig' already has them (squared singular values of the centred,
% scaled design ARE the Gram eigenvalues). The svd below recomputes
% the same p numbers from the full n x p matrix at roughly the cost
% of the whole fit, and allocates another copy of X to do it.
            d = dGram;
        else
            mx = mean(X,1);
            sx = std(X,0,1);
...
% VERIFY, ON THIS SESSION'S OWN DATA, that the fast route and the MATLAB
% function ridge are the same estimator -- and that the CHOICE OF LAMBDA does
% not depend on which of them the cross-validation folds are fitted with.
% Three comparisons, all on a subsample small enough that the slow route is
% at random from this fit's real design matrix):
%   1. COEFFICIENTS across the whole lambda grid.
%   2. FITTED VALUES across the whole lambda grid. This is the comparison that
%      matters -- every number in the paper is built from predictions, not from
%      coefficients, and there is one case (below) where the two can differ.
%   3. THE SELECTED LAMBDA, with the five-fold cross-validation run TWICE over
%      the SAME fold assignment: once with the folds fit by the closed form,
%      once with them fit by ridge(). If the rule lands on the same grid point
%      both ways, the lambda in the paper does not depend on the fast route.
% WHAT IS NOT BEING CHECKED, because it cannot be: MATLAB's ridge does not
% choose lambda. It takes lambda as an input argument. The five-fold rule is the
% one written in the Methods, implemented in selectLambdaCV5, and it is the same
% code in both arms of comparison 3 -- only the fitting route inside the folds
% differs. There is no MATLAB built-in for it to be "the same as".
...
% above against the smallest step between neighbouring grid points on the
...
% THE FILENAME MUST CARRY THE ANIMAL, NOT JUST THE DATE.
% Two animals were recorded on the same day more often than not here --
% TD1d and TD4d share 2023-02-21 and 2023-02-24, TD13d and TD15d share
% 2024-11-24 and 2024-11-25, and TD22d and TD23d share all five June 2025
% dates. With date alone in the name, those 18 sessions wrote to 9 files:
% each one overwrote the other's entry, the key check then correctly
% rejected it, and BOTH reloaded from scratch on every single run, forever,
% name is already in the key -- put it in the file name too.
