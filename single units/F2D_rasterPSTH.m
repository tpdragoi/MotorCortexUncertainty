%% F2D_rasterPSTH.m
%  Rasters and trial-averaged spike rates for example single units, Delayed Reward Task.
...
% ---- condition colours ------------------------------------------------
% Delayed Reward. The CI band is a pale version of the same hue so the
% mean line stays readable on top of it.
pale   = @(c) 1 - 0.55*(1 - c);
col_c1 = [0.05 0.20 0.60];   % R1 dark blue
col_c2 = [0.35 0.70 0.90];   % R4 light blue
