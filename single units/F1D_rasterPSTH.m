%% F1D_rasterPSTH.m
%  Rasters and trial-averaged spike rates for example single units, Simple Reward Task.
...
% ---- condition colours ------------------------------------------------
% Simple Reward. The CI band is a pale version of the same hue so the
% mean line stays readable on top of it.
pale   = @(c) 1 - 0.55*(1 - c);
col_c1 = [0.85 0.10 0.10];   % R1 red
...
r1 = params.trialid{8};
% r4 = params.trialid{9};
idx1 = randperm(numel(r1), 50);
% idx4 = randperm(numel(r4), 50);
trials8 = r1(idx1);
% trials9 = r4(idx4);
...
%          [1 0.6 0.6], 'EdgeColor','none','FaceAlpha',0.9);
...
% 1) Select trials — SAME trials, SAME order, used for ALL THREE panels
r1List = params.trialid{8};
nDraw  = 150;
...
% IDENTICAL position width/height for both raster panels
rasterW = 0.60;
rasterH = 0.24;
rasterL = 0.18;
...
% --- Row 1: Lick raster (black) -------------------------------------------
ax1 = subplot('Position', [rasterL 0.70 rasterW rasterH]);
hold on
for i = 1:nTrials
    trial = sel1(i);
    lickTimes = obj.bp.ev.lickL{trial,1} - obj.bp.ev.goCue(trial);
    plot(lickTimes, i*ones(size(lickTimes)), '.', 'MarkerSize', ms, 'Color','k');
end
...
% --- Row 2: PSTH (red, with 95% CI) + tongue length (gray) — DRAGGABLE ----
...
% build trial-by-trial smoothed rate matrix (T x nTrials)
spkM8 = squeeze(obj.trialdat(:, neuron, trials8));
frM8  = movmean(spkM8, windowSize, 1);

% mean and 95% CI across trials at each time point
mean8 = mean(frM8, 2);
sem8  = std(frM8, 0, 2) ./ sqrt(size(frM8,2));
err8  = 1.96 * sem8;

avg_tLength = kin.dat(:, trials8, 54);
avg_tLength(isnan(avg_tLength)) = 0;
avg_tLength = movmean(mean(avg_tLength,2), 48);

peakRate   = max(mean8);
peakTongue = max(avg_tLength);
tlen_scaled = (avg_tLength / peakTongue) * peakRate;

yMax = max(peakRate + max(err8), max(tlen_scaled)) * 1.15;

% --- shaded 95% CI band for PSTH (drawn first, behind the lines) ----------
hFill = fill([time; flipud(time)], [mean8-err8; flipud(mean8+err8)], ...
    pale(col_c1), 'EdgeColor','none', 'FaceAlpha', 0.6);

hTongue = plot(time, tlen_scaled, '-', 'Color',[0.6 0.6 0.6], 'LineWidth', 2.5);
hRate   = plot(time, mean8, '-', 'Color', col_c1, 'LineWidth', 2);

xline(0, '--k', 'LineWidth', 1);
xlim(xLimVals);
ylim([0, yMax])

text(-0.32, yMax*0.5, 'Rate', 'Color', col_c1, 'FontWeight','bold', ...
    'FontSize', 13, 'Rotation', 90, 'HorizontalAlignment','center')
text(0.45, yMax*0.95, 'Tongue Length', 'Color',[0.6 0.6 0.6], ...
    'FontSize', 12, 'FontWeight','bold')

yScale = 10;
plot([sbX sbX], [0, yScale], 'r-', 'LineWidth', 2.5)
text(sbX+0.05, yScale/2, '10 Hz', 'Color','r', 'FontSize', 10, ...
    'HorizontalAlignment','left', 'VerticalAlignment','middle')

axis off

% make rate trace AND its CI fill draggable together; tongue trace separately
makeLineDraggable(hRate, hFill);
makeLineDraggable(hTongue, []);

% --- Row 3: Spike raster (red) — IDENTICAL size/ylim to Row 1 -------------
ax3 = subplot('Position', [rasterL 0.10 rasterW rasterH]);
hold on
for i = 1:nTrials
    trial = sel1(i);
    spkTimes = time(obj.trialdat(:, neuron, trial)~=0);
    plot(spkTimes, i*ones(size(spkTimes)), '.', 'MarkerSize', ms, 'Color','r');
end
xline(0, '--k', 'LineWidth', 1);
xlim(xLimVals);
ylim(rasterYLim);   % EXACTLY same as ax1

text(-0.32, nTrials/2, 'Spikes', 'Color','r', 'FontWeight','bold', ...
    'FontSize', 13, 'Rotation', 90, 'HorizontalAlignment','center')

plot([sbX sbX], [0, rasterScale], 'r-', 'LineWidth', 2.5)
text(sbX+0.05, rasterScale/2, [num2str(rasterScale) ' trials'], 'Color','r', ...
    'FontSize', 10, 'HorizontalAlignment','left', 'VerticalAlignment','middle')
...
sgtitle(['Neuron ', num2str(nn)], 'FontSize', 12)

end

% Helper: make a line (and optionally its CI fill) draggable as a unit
function makeLineDraggable(hLine, hFill)
    origY = hLine.YData;
    if ~isempty(hFill)
